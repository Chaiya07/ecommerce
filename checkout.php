...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        foreach ($_SESSION['cart'] as $item) {
            ...
            $sqlStock = $conn->prepare('UPDATE products SET stock = stock - ? WHERE id = ?');
            $sqlStock->execute([$item['qty'], $item['id']]);
        }

        if (!empty($_SESSION['coupon'])) {
            $sqlCoupon = $conn->prepare('UPDATE coupons SET used_count = used_count + 1 WHERE id = ?');
    ...
    <div class="col-md-5" data-aos="fade-up" data-aos-delay="100">
        <div class="card order-card">
            ...
            <div class="card-body">
                ...
                <p>ส่วนลด :
                    <strong class="text-danger"><?= number_format($discount, 2) ?></strong> บาท
                </p>
                <hr>
                <p>ยอดสุทธิ :
                    <strong class="text-success fs-4"><?= number_format($finalTotal, 2) ?></strong> บาท
                </p>
            </div>
        </div>
    </div>
