...
$sqlItems = $conn->prepare('SELECT oi.*, p.name, p.image FROM order_items oi
...
<div class="card mb-4">
    <div class="card-header">รายละเอียดคำสั่งซื้อ</div>
    <div class="card-body">
        <div class="row">
            <div class="col-md-6">
                <p><strong>เลขที่คำสั่งซื้อ :</strong> #<?= $order['id'] ?></p>
                <p><strong>ชื่อผู้สั่งซื้อ :</strong> <?= htmlspecialchars($order['customer_name']) ?></p>
                <p><strong>อีเมล :</strong> <?= htmlspecialchars($order['customer_email']) ?></p>
...
<div class="card mb-4">
    <div class="card-header">ที่อยู่จัดส่ง</div>
    <div class="card-body">
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    <?php foreach ($items as $item) : ?>
                        <?php $image = !empty($item['image']) ? 'uploads/products/' . $item['image'] : ''; ?>
                        <tr>
...
<div class="mt-4">
    <button class="btn btn-secondary" onclick="window.print()">พิมพ์ใบสั่งซื้อ</button>
    <a href="index.php" class="btn btn-primary">กลับหน้าหลัก</a>
    <?php if (!empty($_SESSION['user'])) : ?>
        <a href="my_orders.php" class="btn btn-success">ประวัติคำสั่งซื้อ</a>
    <?php endif; ?>
</div>
